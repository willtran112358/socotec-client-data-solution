...
flowchart TB
    ...
    CONTRACT --> PROJECT
    INSPECTOR[Inspector] --> INSPECTION[Inspection]

    classDef entity fill:#E3F2FD,stroke:#1565C0,stroke-width:1px,color:#0D47A1;
    class CUSTOMER,PROJECT,SITE,ASSET,CONTRACT,INSPECTOR,INSPECTION entity;
```

Customer and asset master relationships support contract, project, and inspection lineage.


### 4.2 Inspection, Findings, Actions, Compliance
```mermaid
flowchart TB
    ASSET[Asset] --> INSPECTION[Inspection]
    INSPECTION --> INSPECTION_FINDING[Finding]
    INSPECTION_FINDING --> WORK_ORDER[Work Order]
    INSPECTION --> COMPLIANCE_RESULT[Compliance Result]
    REGULATION_RULE[Regulation Rule] --> COMPLIANCE_RESULT
    WORK_ORDER --> WORK_ORDER_EVENT[Work Order Event]

    classDef entity fill:#E3F2FD,stroke:#1565C0,stroke-width:1px,color:#0D47A1;
    class ASSET,INSPECTION,INSPECTION_FINDING,WORK_ORDER,COMPLIANCE_RESULT,REGULATION_RULE,WORK_ORDER_EVENT entity;
```

Inspection workflows are mapped to findings, action items, compliance checks, and follow-up events.
